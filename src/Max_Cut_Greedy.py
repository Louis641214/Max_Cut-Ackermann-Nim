...
def max_cut_greedy(g: Graph[T]) -> Tuple[Dict[Vertex[T], Literal[0,1]], float]:
    ...
    while improved:
        ...
        for vertex in g.vertices:
            current_cut = compute_sum_cut_value(g, partition)
            partition[vertex] = 1 - partition[vertex] # Moving the vertex to the other group
            new_cut = compute_sum_cut_value(g, partition)
...
def compute_sum_cut_value(g: Graph[T], partition: Dict[Vertex[T], Literal[0,1]]) -> float:
    sum_cut_value = 0
    for u, v, weight in g.edges:
        if partition[u] != partition[v]:
            sum_cut_value += weight
    return sum_cut_value
